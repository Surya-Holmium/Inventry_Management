...
class ComboBoxLoader(QThread):
    ...
    def run(self):
        try:
            response = requests.get(f"{SERVER_URL}/employee/{self.itemOrEmpl}")
            if response.ok:
...
class IssueStocksWindow(QDialog):
    def __init__(self, uName, item_id, itemName, on_issue=None):
        super().__init__()
        self.loaders = []
        self.userName = uName
        ...
        self.issuedby_input = self.create_input_row(form_layout, "Issued By:")
    ...
    def create_input_row(self, layout, label_text):
        ...
        if label_text == "Item Name:":
            ...
            input_field.setText(self.userName)
        else:
